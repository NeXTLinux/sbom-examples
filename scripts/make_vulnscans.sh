...
do
    ...
    if [ -f ./results/${ODIR_IMAGE}/sbom/gosbomjson.json ]; then
	if ( mkdir -p ./results/${ODIR_IMAGE}/vulnscan ); then
	    if ( ./grype -q -o json ./results/${ODIR_IMAGE}/sbom/gosbomjson.json > ./results/${ODIR_IMAGE}/vulnscan/grypejson.json ); then
		./grype -q -o cyclonedx ./results/${ODIR_IMAGE}/sbom/gosbomjson.json > ./results/${ODIR_IMAGE}/vulnscan/cyclonedx.json
	    else
